...
class CalonSiswa_Model extends CI_Model
{
    ...
    {
        $this->db->select('c.*,p.alamat as alamat,t.nilai,t.status');
        $this->db->from('calonsiswa c');
        $this->db->join('pendaftaran p', 'p.id_siswa = c.id_siswa', 'left');
        $this->db->join('tes t', 't.username = c.username', 'left');
        $query = $this->db->get();
        return $query->result_array();
    }
    ...
    {
        $query = 'SELECT * FROM calonsiswa c, dokumen d, pendaftaran p where c.id_siswa = p.id_siswa and p.status = "Lengkap" GROUP BY p.id_siswa';
        return $this->db->query($query)->result_array();
    }

    public function get_tolak()
    {
        $query = 'SELECT * FROM calonsiswa c, dokumen d, pendaftaran p where c.id_siswa = p.id_siswa and p.status = "Belum Lengkap" GROUP BY p.id_siswa';
        return $this->db->query($query)->result_array();
    }
    ...
    {
        $query = 'SELECT * FROM calonsiswa c, pendaftaran p, tes t where c.username = t.username and t.status = "Lulus" GROUP BY t.username';
        return $this->db->query($query)->result_array();
    }

    public function get_tidak_lulus()
    {
        $query = 'SELECT * FROM calonsiswa c, pendaftaran p, tes t where c.username = t.username and t.status = "Tidak Lulus" GROUP BY t.username';
        return $this->db->query($query)->result_array();
    }

    public function get_belum_verifikasi()
    {
        $query = 'SELECT * FROM calonsiswa c, pendaftaran p, tes t where c.username = t.username and t.status = "Belum Diverifikasi" GROUP BY t.username';
        return $this->db->query($query)->result_array();
    }
    ...
    public function get_nilai_id($username)
    {
        $query = 'SELECT nilai from tes where username = "' . $username . '"';
        return $this->db->query($query)->result();
    }
    public function get_tes_id($username)
    {
        $query = 'SELECT status from tes where username = "' . $username . '"';
        return $this->db->query($query)->result();
    }
